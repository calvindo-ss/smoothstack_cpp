...
void c1trades(std::chrono::steady_clock::time_point begin, Country& c1, Country& c2, Request r1, Request r2, Request r3, Request r4){
    double time;
    while(time < 10){
        int c = (std::rand() % 4);
        switch(c){
...
void c2trades(std::chrono::steady_clock::time_point begin, Country& c1, Country& c2, Request r1, Request r2, Request r3, Request r4){
    double time;
    while(time < 10){
        int c = (std::rand() % 4);
        switch(c){
...
int main(void){
    ...
    std::thread t1(c1trades, begin,std::ref(c1), std::ref(c2), r1, r2, r3, r4);
    std::thread t2(c2trades, begin, std::ref(c2), std::ref(c1), r1, r2, r3, r4);
